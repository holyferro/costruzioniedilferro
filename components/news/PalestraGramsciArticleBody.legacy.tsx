...
import { palestraGramsciArticle } from "@/content/articles/palestra-gramsci";
import { LeadBlock, SectionBlock, SpecsBlock, CtaBlock } from "@/components/article/blocks";
import type { ArticleBlock } from "@/lib/types/article";

export function PalestraGramsciArticleBody() {
  const article = palestraGramsciArticle;
    ...
    <article>
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to bottom, rgba(10,24,48,0.25) 0%, rgba(10,24,48,0.45) 50%, rgba(10,24,48,0.9) 100%)",
          }}
        />
                ...
                if (block.type === "paragraph") {
                  if (block.html) {
                    return (
                      <div
                        key={bi}
                        className="text-ink/80 mt-8 text-[17px] leading-[1.75]"
                        dangerouslySetInnerHTML={{ __html: block.text }}
                      />
                    );
                  }
                  return (
                    <p key={bi} className="text-ink/80 mt-8 text-[17px] leading-[1.75]">
                      {block.text}
                    </p>
                  );
                }
