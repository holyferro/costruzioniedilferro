# Stack Research — Impresa Edile SRL Website
...
The user's CLAUDE.md already locks in the right foundation for 2026: **Next.js 16 App Router + TypeScript strict + Tailwind v4 + Vercel + pnpm**. This research validates those choices and fills in the rest of the stack with the minimum viable set of libraries needed for a credible, performant, lead-generating institutional site — no headless CMS, no heavy animation framework, no client-side state libraries. Content is static MDX/TS, forms use React Hook Form + Zod + Server Actions, email goes through Resend, spam protection is Cloudflare Turnstile, analytics is Vercel Analytics + Speed Insights (GDPR-safe), and structured data is type-safe via `schema-dts`.
...
**No flags.** Every locked-in decision holds in 2026.
...
pnpm create next-app@latest edilferro-site \
...
pnpm add react-hook-form zod @hookform/resolvers resend @marsidev/react-turnstile
...
**If the client later asks to self-manage the projects gallery:**
...
**If the client wants Italian + English bilingual:**
...
**If Core Web Vitals underperform on mobile despite Next 16 defaults:**
...
**If Turnstile ever blocks legitimate Italian professional visitors (edge case):**
...
**Italian-context sources (HIGH confidence):**
...
**Secondary / comparative (MEDIUM confidence — used for tradeoff analysis, not primary claims):**
...
*Stack research for: institutional corporate website / lead generation — local Italian construction company*
*Researched: 2026-04-13*
*Overall confidence: HIGH*
